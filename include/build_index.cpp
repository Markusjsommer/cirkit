...
void build_index::extract_features(const std::string& feature, bool noalt) {
    ...
    for (auto & x: _gtf_data){
        ...
        _gtf_seq_forward.emplace_back(seq_forward);
...
void build_index::build(int k, int m) {
    ...
    for (uint32_t i=0; i < _gtf_data.size(); ++i){
        ...
        std::string *seq_f = &_gtf_seq_forward[i];
        std::string *seq_fc = &_gtf_seq_forwardcomplement[i];
        ...
        if (*(void**)seq_f != nullptr or *(void**)seq_fc != nullptr){ // check NULL
            if (seq_f->size() <= k){ // skip sequences below length of kmer
                continue;
            }
        } else {
            continue;
        ...
        for (uint32_t j=0; j < seq_f->size() - k; ++j){
            kmer = seq_f->substr(j, k);
            ...
            if (geneIDpos != bm.geneID_array.end()){
                ...
                geneIDidx = bm.geneID_array.size() - 1; //TODO ensure not obo
            }
            bm.setbits(kmer_bitset, geneIDidx, 0, 16);
            // 16 : strand
            if (strand){
                bm.setbits(kmer_bitset, 1, 16, 17);
            }
            // 17:32 : contig
            uint64_t contigidx;
            ...
            uint64_t pos = seq_start_position + j;
            bm.setbits(kmer_bitset, pos, 32, 64);

            // add 64 bits to table
        ...
        // TODO BE VERY CONFIDENT ON EXACTLY HOW TO HANDLE COMPLEMENTS AND STRANDS
        // insert kmers from REVERSE complement
        // position is of 5' end

        // REVERSE THE FORWARD COMPLEMENT TO GET REVERSE COMPLEMENT
        std::reverse(seq_fc->begin(), seq_fc->end());

        for (uint32_t j=0; j < seq_fc->size() - k; ++j){
            kmer = seq_fc->substr(j, k);
//            kmer_info = std::make_tuple(contig, seq_end_position - j, strand, geneID); // TODO ensure no off by 1 error here
